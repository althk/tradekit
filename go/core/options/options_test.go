...
// nifty is the model zerobha runs; the numbers below were computed with it.
var nifty = Model{RiskFree: 0.065, DivYield: 0.012}
...
func TestImpliedVolRoundTrip(t *testing.T) {
	...
	for _, years := range []float64{1.0 / 365, 3.0 / 365, 7.0 / 365} {
		for _, want := range []float64{0.08, 0.12, 0.25, 0.60} {
			price := nifty.Price(s, k, years, want, true)
			got, ok := nifty.ImpliedVol(price, s, k, years, true)
			if !ok {
...
func TestImpliedVolRejectsImpossiblePrice(t *testing.T) {
	if _, ok := nifty.ImpliedVol(1, 24300, 24300, 2.0/365, true); ok {
		t.Error("accepted a price below the lowest the model can produce")
	}
	if _, ok := nifty.ImpliedVol(24300, 24300, 24300, 2.0/365, true); ok {
	...
	for _, years := range []float64{0.5 / 365, 2.0 / 365, 6.0 / 365} {
		for _, target := range []float64{0.55, 0.65, 0.80, 0.90} {
			for _, isCall := range []bool{true, false} {
				k := nifty.StrikeForDelta(s, years, sigma, target, isCall)
				got := math.Abs(nifty.Delta(s, k, years, sigma, isCall))
...
func TestStrikeForDeltaWidensWithTimeToExpiry(t *testing.T) {
	...
	near := s - nifty.StrikeForDelta(s, 1.0/365, sigma, target, true)
	far := s - nifty.StrikeForDelta(s, 6.0/365, sigma, target, true)
	if !(far > near*1.5) {
